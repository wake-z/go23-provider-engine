...
function HookedWalletSubprovider(opts){
  ...
  if (opts.getWakeAccounts) self.getWakeAccounts = opts.getWakeAccounts
  // high level override
...
HookedWalletSubprovider.prototype.handleRequest = function(payload, next, end){
  ...
  switch(payload.method) {
    ...
    case 'eth_accounts':
      // process normally
      self.getWakeAccounts(function(err, accounts){
        if (err) return end(err)
        end(null, accounts)
      })
      return
...
HookedWalletSubprovider.prototype.getWakeAccounts = function(cb) {
  cb(null, [])
}
